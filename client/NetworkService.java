...
public class NetworkService {
    ...
    public static void sendReg(String login, String password, String nick) {
        try {
            if (socket == null || socket.isClosed()) {
                connect();
            }
        ...
        try {
            ...
            Thread clientListener = new Thread(() -> {
                try {
                    ...
                    while (true) {
                        msg = in.readUTF();
                        if (msg.startsWith("/nickIsBusy ")){
                            Alert alert = new Alert(Alert.AlertType.ERROR); // Выдать окно с ошибкой, если такой никнейм уже есть
                            alert.setTitle("Данный никнейм уже занят");
                            alert.showAndWait();
                        }
